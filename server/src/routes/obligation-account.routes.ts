...
function isAutoGeneratedInstallmentTitle(title: string) {
  return /^Cuota \d+ de \d+$/i.test(title.trim())
}
...
function buildObligationTotals(obligation: {
  ...
  const nextStatus: 'open' | 'settled' =
    remainingAmount <= 0.01 ? 'settled' : 'open'
...
function buildInstallmentRecalculationPayload(params: {
  obligations: Array<{
    id: string
    title: string
  ...
  const { obligations, loanTotalAmount, installmentCount } = params

  if (obligations.length !== installmentCount) {
    return {
      error:
        'No se pudo recalcular automaticamente la cuenta porque la cantidad de obligaciones cargadas no coincide con las cuotas pactadas. Corrige las obligaciones manualmente o recrea la cuenta.',
    }
  }

  const nextPrincipalAmounts = splitAmountAcrossInstallments(
    loanTotalAmount,
    installmentCount
  )

  const recalculatedObligations: Array<{
    id: string
    data: {
      title: string
      referenceMonth: string
  ...
  for (const [index, obligation] of obligations.entries()) {
    const nextPrincipalAmount = roundAmount(nextPrincipalAmounts[index] ?? 0)
    ...
    if (nextTotals.paidAmount > nextTotals.totalAmount + 0.01) {
      return {
        error: `No se pudo recalcular la obligacion "${obligation.title}" porque ya tiene abonos por ${nextTotals.paidAmount.toLocaleString(
          'es-AR',
          {
            minimumFractionDigits: 2,
            maximumFractionDigits: 2,
          }
        )} y el nuevo total quedaria por debajo de ese importe.`,
      }
    }

    recalculatedObligations.push({
      id: obligation.id,
      data: {
        title: isAutoGeneratedInstallmentTitle(obligation.title)
          ? `Cuota ${index + 1} de ${installmentCount}`
          : obligation.title,
        referenceMonth:
          obligation.referenceMonth ??
          buildReferenceMonthFromDate(obligation.dueDate),
        principalAmount: nextPrincipalAmount,
...
router.put('/:accountId', authMiddleware, async (req: AuthRequest, res) => {
  try {
    ...
    if (
      existingAccount.obligations.length > 0 &&
      existingAccount.loanTotalAmount !== null &&
      existingAccount.installmentCount !== null &&
      (validation.data.loanTotalAmount === null ||
        validation.data.installmentCount === null)
    ) {
      return res.status(400).json({
        ok: false,
        message:
          'No puedes vaciar el monto total o la cantidad de cuotas mientras esta cuenta tenga obligaciones cargadas.',
      })
    }

    const nextLoanTotalAmount = validation.data.loanTotalAmount
    const nextInstallmentCount = validation.data.installmentCount

    const shouldRecalculateInstallments =
      existingAccount.obligations.length > 0 &&
      supportsInstallmentPlanType(validation.data.type) &&
      nextLoanTotalAmount !== null &&
      nextInstallmentCount !== null &&
      (nextLoanTotalAmount !== existingAccount.loanTotalAmount ||
        nextInstallmentCount !== existingAccount.installmentCount)

    const recalculatedInstallments = shouldRecalculateInstallments
      ? buildInstallmentRecalculationPayload({
          obligations: existingAccount.obligations,
          loanTotalAmount: nextLoanTotalAmount,
          installmentCount: nextInstallmentCount,
        })
      : null

    if (recalculatedInstallments && 'error' in recalculatedInstallments) {
      return res.status(400).json({
        ok: false,
        message: recalculatedInstallments.error,
      })
    }

    await prisma.$transaction(async (transactionClient) => {
      await transactionClient.obligationAccount.update({
        where: {
          id: accountId,
        },
      ...
      if (recalculatedInstallments && 'data' in recalculatedInstallments) {
        await Promise.all(
          recalculatedInstallments.data.map((obligation) =>
            transactionClient.obligation.update({
              where: {
                id: obligation.id,
              },
